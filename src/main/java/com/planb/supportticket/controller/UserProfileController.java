...
public class UserProfileController {
     ...
     *
     * @param userProfileDTO the updated user profile
     * @return the updated user profile
     */
    @PutMapping("/profile")
    public ResponseEntity<?> updateCurrentUserProfile(@RequestBody UserProfileDTO userProfileDTO) {
        try {
            // Get the current user's ID from the security context
            Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
            String userId;
            ...
            log.error("Error updating current user profile: {}", e.getMessage());
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    }
}
